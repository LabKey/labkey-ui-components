...
describe("changeColumn", () => {
    ...
    test("has values and messages", () => {
        ...
        const newColIndex = updatedGridModel.queryInfo.columns.keySeq().findIndex((column) => column === queryColumn.fieldKey.toLowerCase());
...
describe("addColumns", () => {
    ...
    test("add at end", () => {
        ...
        expect(updatedGridModel.getColumnIndex(queryColumn.fieldKey)).toBe(queryGridModel.getColumnIndex(lastInsertColKey) + 1);
        expect(updatedGridModel.data.findEntry((rowValues) => rowValues.has(queryColumn.fieldKey))).toBeTruthy();
