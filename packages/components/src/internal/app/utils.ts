...
import {
    ASSAYS_KEY, HOME_KEY, LOCATIONS_KEY, NEW_ASSAY_DESIGN_HREF, NEW_FREEZER_DESIGN_HREF,
    NEW_SAMPLE_TYPE_HREF,
    NEW_SOURCE_TYPE_HREF, SAMPLES_KEY,
    SECURITY_LOGOUT,
...
export function isFreezerManagementEnabled(): boolean {
    ...
    return getServerContext().moduleContext.inventory && (!smEnabled ||
        getServerContext().moduleContext.samplemanagement.hasFreezerManagementEnabled === true);
}
...
export function getMenuSectionConfigs(user: User, currentApp: string) : List<Map<string, MenuSectionConfig>> {
    ...
    if (isSampleManagerEnabled()) {
        ...
        workflowAndUserSectionConfig = workflowAndUserSectionConfig.set(WORKFLOW_KEY, new MenuSectionConfig({
        ...
        workflowAndUserSectionConfig = workflowAndUserSectionConfig.set(USER_KEY, new MenuSectionConfig({
...
function getApplicationUrlBase(moduleName: string, currentApp: string) : string {
    ...
    return (!appName || appName === currentApp.toLowerCase()) ? '' : buildURL(appName, "app.view", undefined, {returnURL: false});
}
