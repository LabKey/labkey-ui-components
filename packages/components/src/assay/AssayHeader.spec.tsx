...
describe("AssayHeader", () => {
    ...
    function validate(wrapper: ReactWrapper, title: string, subTitle: string, description: string, titleActiveIndicator: boolean = false, descriptionActiveIndicator: boolean = false)
    {
    ...
    test("active job, name as description", () => {
        const wrapper = mountWithAppServerContext(
            <AssayContextProvider value={{
                ...
                <AssayHeader menu={menuWithJobInProgress} title="Batch" description={TEST_ASSAY_STATE_MODEL.definitions[1].name}/>
            </AssayContextProvider>
        );
