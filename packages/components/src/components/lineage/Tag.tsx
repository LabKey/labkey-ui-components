...
export const Tag: React.FunctionComponent<TagProps> = (props) => (
    <span className={'pull-right ' + (props.showOnHover === false ? 'hide-on-hover' : 'show-on-hover')}>
        <span className={'label ' + (props.bsStyle ? `label-${props.bsStyle}` : '')}>
            {props.children}
        </span>
    </span>
);
...
export const LINEAGE_GRID_COLUMNS = List([
    new GridColumn({
        ...
        cell: (name: string, node: Map<string, any>) => {
            ...
            return (
                <div className="lineage-name" style={{textIndent: indent + "px"}} title={dupeCount > 0 ? dupeCount + " duplicates" : ""}>
                    {node.get('url') ? <a href={node.get('url')}>{name}</a>: name}
                    {nodeDistance === 0 && (
                        <Tag bsStyle="success">Seed</Tag>
                    )}
                    {nodeDistance === 1 && (
                        <Tag bsStyle="info">1st {isParents ? "parent" : "child"}</Tag>
                    )}
                    {nodeDistance === 2 && (
                        <Tag bsStyle="primary">2nd {isParents ? "parent" : "child"}</Tag>
                    )}
                    {dupeCount > 0 && (
                        <Tag bsStyle="warning" showOnHover={false}>Duplicate</Tag>
                    )}
                </div>
            );
        }
