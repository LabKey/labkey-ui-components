...
const ASSAY_EMPTY_ALERT = '.assay-design-empty';
...
describe('AssayDesignEmptyAlert', () => {
    test('with permissions', () => {
        ...
        expect(wrapper.find(ASSAY_EMPTY_ALERT).at(0).text()).toContain('No assays have been created.');

        // Expect link to design
        expect(wrapper.find(`${ASSAY_EMPTY_ALERT} a`).prop('href')).toEqual(App.NEW_ASSAY_DESIGN_HREF.toHref());
    });
    test('without permissions', () => {
        ...
        expect(wrapper.find(ASSAY_EMPTY_ALERT).at(0).text()).toEqual(expectedMessage);
    });
});
