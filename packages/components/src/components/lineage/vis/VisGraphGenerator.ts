...
export function createLineageNodeCollections(nodes: LineageNode[]): { [key: string]: LineageNodeCollection } {
    ...
    for (const groupName of Object.keys(nodesByType)) {
        ...
        group.listURL = createNodeListURL(group.baseListURL, group.nodes);
    }

    return nodesByType;
}

function createNodeListURL(listURL: string, nodes: LineageNode[]) {
...
export interface VisGraphCombinedNode extends Node {
    ...
    containedNodesByType: { [key: string]: LineageNodeCollection };
}
...
export function isCombinedNode(
    item: VisGraphNode | VisGraphCombinedNode | VisGraphClusterNode
): item is VisGraphCombinedNode {
    return item.kind === 'combined';
}

export function isClusterNode(
    item: VisGraphNode | VisGraphCombinedNode | VisGraphClusterNode
): item is VisGraphClusterNode {
...
export function generate(result: LineageResult, grouping?: ILineageGroupingOptions): VisGraphOptions {
    ...
    if (grouping === undefined) grouping = new LineageGroupingOptions();
...
function processNodes(
    ...
    options: ILineageGroupingOptions,
...
function _processNodes(
    ...
    if (options) {
        ...
        if (
            options.generations === LINEAGE_GROUPING_GENERATIONS.Specific &&
            depth + 1 > (dir === LINEAGE_DIRECTIONS.Parent ? options.parentDepth : options.childDepth)
        ) {
    ...
    if (edges.size > 0) {
        ...
        if (options.combineSize && edges.size >= options.combineSize) {
            ...
            const combinedNode = createCombinedVisNode(nodes, containedNodes, options.combineSize);
...
function createVisNode(node, id, isSeed: boolean, isCombined): VisGraphNode {
    ...
    const image = getImageFromLineageNode(node, isSeed, false);
    const selectedImage = getImageFromLineageNode(node, isSeed, true);
    const imageBackup = getBackupImageFromLineageNode(node, isSeed, false);
...
function createCombinedVisNode(
    ...
    const id = 'combined:' + randId();

    let typeLabel;
    let commonNode: LineageNode;
    const containedNodes: LineageNode[] = [];

    for (let i = 0; i < containedNodeIds.length; i++) {
        const containedNode = nodes.get(containedNodeIds[i]);
        containedNodes.push(containedNode);
    }

    const containedNodesByType = createLineageNodeCollections(containedNodes);
    ...
    if (commonNode) {
        // show the alternate icon image color if this node is the seed or has been selected
        const image = getImageFromLineageNode(commonNode, false, false);
        const selectedImage = getImageFromLineageNode(commonNode, false, true);
        const imageBackup = getBackupImageFromLineageNode(commonNode, false, false);
