...
describe('SampleAmountEditModal', () => {
    ...
    function validate(wrapper: ReactWrapper, amount: number, units: string, hasSelect: boolean, comment: string, noun: string, canSave: boolean, isNegative?: boolean, hasLabelUnits: boolean = true) {

        expect(wrapper.find(".checkin-amount-label").text()).toContain("Amount" + (units && hasLabelUnits ? " (" + units + ")" : ""));
        ...
        if (isNegative)
            expect(wrapper.find(Alert).at(1).text()).toBe("Amount must be a positive value.");
    ...
    test("minimal props", () => {
        ...
        const wrapper = mountWithServerContext(
            <SampleAmountEditModal schemaQuery={testSchemaQuery} row={emptyRow} noun={noun} updateListener={jest.fn()} onClose={jest.fn()} />,
    ...
    test("Amount null", () => {
        ...
        const wrapper = mountWithServerContext(
            <SampleAmountEditModal schemaQuery={testSchemaQuery} row={row} noun={noun} updateListener={jest.fn()} onClose={jest.fn()} />,
            {user: TEST_USER_EDITOR}
        );
    ...
    test("Set comment and StoredAmount, can save", () => {
        ...
        wrapper.find("#userComment").simulate('change', { target: { name: 'body', value: userComment } });
        wrapper.find("input.storage-amount-input").simulate('change', { target: { name: 'amountDelta', value: newStoredAmount } });
