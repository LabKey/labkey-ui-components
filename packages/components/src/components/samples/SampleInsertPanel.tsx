...
export class SampleInsertPanelImpl extends React.Component<Props, StateProps> {
    ...
    init(props: OwnProps, selectTab = false) {
        const queryParams = props.location
            ? SampleInsertPanelImpl.getQueryParameters(props.location.query)
            : {
                  parents: undefined,
        ...
        const tab =
            props.location && props.location.query && props.location.query.tab
                ...
                : SampleInsertPanelTabs.Grid;
        if (selectTab && tab != SampleInsertPanelTabs.Grid) this.props.selectStep(parseInt(tab));
                    ...
                    this.setState(() => {
                        return {
                            insertModel: insertModel.merge({
                                isError: true,
                                errors:
                                    "Problem retrieving data for sample type '" +
                                    insertModel.getTargetSampleSetName() +
                                    "'.",
        ...
        if (insertModel) {
            ...
            if (sampleSetName) {
                ...
                const model = getStateQueryGridModel(
                    'insert-samples',
                    SchemaQuery.create(SCHEMAS.SAMPLE_SETS.SCHEMA, sampleSetName),
                    {
                        editable: true,
    ...
    getGridQueryInfo(): QueryInfo {
        ...
        if (originalQueryInfo) {
            const nameIndex = Math.max(
                0,
                originalQueryInfo.columns.toList().findIndex(column => column.fieldKey === SAMPLE_UNIQUE_FIELD_KEY)
            );
            const newColumnIndex =
                nameIndex + insertModel.sampleParents.filter(parent => parent.query !== undefined).count();
        ...
        if (queryGridModel) {
            ...
            if (parent) {
                ...
                updatedModel = insertModel.mergeIn(
                    ['sampleParents', existingParentKey],
                    parentType
                ) as SampleIdCreationModel;
                column = SampleInsertPanelImpl.generateParentColumn(parentType);
            } else {
                const parentToResetKey = insertModel.sampleParents.findKey(parent => parent.get('index') === index);
                ...
                updatedModel = insertModel.mergeIn(
                    ['sampleParents', parentToResetKey],
                ...
                () => {
                    if (column && existingParent) {
                        if (existingParent.query !== undefined) {
                            ...
                            let fieldKey;
                            if (existingParent.index === 1) fieldKey = SAMPLE_UNIQUE_FIELD_KEY;
                            else {
                                const definedParents = insertModel.sampleParents.filter(
                                    parent => parent.query !== undefined
                                );
                                if (definedParents.size === 0) fieldKey = SAMPLE_UNIQUE_FIELD_KEY;
                                else {
                                    ...
                                    fieldKey = prevParent
                                        ? this.createParentColumnName(prevParent)
    ...
    removeParent(index) {
        const { insertModel } = this.state;
        const parentToResetKey = insertModel.sampleParents.findKey(parent => parent.get('index') === index);
        const parentColumnName = this.createParentColumnName(insertModel.sampleParents.get(parentToResetKey));
                    ...
                    <>
                        ...
                        {parentOptions.size > sampleParents.size ? (
                            <div className="sample-insert--header">
                                <AddEntityButton entity="Parent" onClick={this.addParent} />
                            </div>
                        ) : (
                            <div className="sample-insert--header">
                                Only {parentOptions.size} parent{' '}
                                {parentOptions.size === 1 ? 'sample type' : 'sample types'} available.
                            </div>
                        )}
                    </>
                );
    ...
    renderHeader(isGrid: boolean) {
        ...
        return (
            <>
                {isGrid && (
                    <div className="sample-insert--header">
                        <p>Generate unique samples individually or in bulk using the bulk insert option.</p>
                    </div>
                )}
                {insertModel.isInit && (
                    <SelectInput
                        ...
                        required
                        value={
                            insertModel && insertModel.hasTargetSampleSet()
                                ? insertModel.targetSampleSet.label
                                : undefined
                        }
                    />
                )}
                {insertModel.isError
                    ? this.renderError()
                    : isGrid && insertModel.hasTargetSampleSet()
                    ? this.renderParentSelections()
                    : ''}
            </>
        );
    }
    ...
    insertRowsFromGrid() {
        ...
        insertModel
            .postSampleGrid(this.getQueryGridModel())
            .then((response: InsertRowsResponse) => {
                if (response && response.rows) {
                    ...
                    if (this.props.afterSampleCreation) {
                        this.props.afterSampleCreation(
                            insertModel.getTargetSampleSetName(),
                            response.getFilter(),
    ...
    deriveSampleIds(count: number) {
        ...
        insertModel
            .deriveSamples(count)
            .then((result: GenerateSampleResponse) => {
                this.setSubmitting(false);
                if (this.props.onDataChange) {
                    this.props.onDataChange(false);
                }
                if (this.props.afterSampleCreation) {
                    this.props.afterSampleCreation(
                        insertModel.getTargetSampleSetName(),
                        result.getFilter(),
                        result.data.materialOutputs.length,
                        'created'
                    );
                }
            })
    ...
    renderCreateFromGrid() {
        ...
        if (!this.isNameRequired()) {
            columnMetadata = columnMetadata.set(SAMPLE_UNIQUE_FIELD_KEY, {
                ...
                toolTip:
                    "A generated sample ID will be provided for samples that don't have a user-provided ID in the grid.",
            });
        } else {
            columnMetadata = columnMetadata.set(SAMPLE_UNIQUE_FIELD_KEY, {
                toolTip:
                    'A sample ID is required for each sample since this sample type has no naming pattern. You can provide a naming pattern by editing the sample type details.',
            });
        }
            ...
            <>
                ...
                <div className="top-spacing">
                    {queryGridModel && queryGridModel.isLoaded ? (
                        ...
                        />
                    ) : !insertModel.isError && insertModel.targetSampleSet && insertModel.targetSampleSet.value ? (
    ...
    importOptionHelpText() {
        return (
            <>
                ...
                <p>
                    When update is selected, data will be updated for matching Sample IDs, and new samples will be
                    created for any new Samples IDs provided. Data will not be changed for any columns not in the
                    imported file.
                </p>
                <p>
                    For more information on import options for samples, see the{' '}
                    {helpLinkNode(IMPORT_SAMPLE_SETS_TOPIC, 'Import Sample Types')} documentation page.
                </p>
            </>
        );
    }
    ...
    renderImportOptions() {
        return (
            <div className="margin-bottom">
                ...
                <span className="sm-mergeoption-checkbox" onClick={this.toggleInsertOptionChange}>
                ...
                <div className="panel panel-default">
                    <div className="panel-body">
                        <div className="row">
                            ...
                            {editSampleTypeDetailsLink && canEditSampleTypeDetails ? (
                                <div className="col-sm-5">
                                    <Link
                                        className="pull-right sample-insert--link"
                                        to={editSampleTypeDetailsLink.toString()}>
                                        Edit Sample Type Details
                                    </Link>
                                </div>
                            ) : (
                                undefined
