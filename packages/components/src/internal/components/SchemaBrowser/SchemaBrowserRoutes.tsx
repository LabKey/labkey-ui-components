...
import { DropdownButton, MenuDivider, MenuHeader, MenuItem, SplitButton } from '../../dropdowns';
...
const DynamicMenuItem: FC<DynamicMenuItemProps> = ({ disabled = false, item, lastClicked, onClick }) => {
    const onClick_ = useCallback(() => onClick(item), [onClick, item]);
    return (
        <MenuItem disabled={disabled} active={item === lastClicked} onClick={onClick_}>
            Dynamic Item {item}
        </MenuItem>
    );
};
...
const MenuExamples = () => {
    ...
    const dynamicOnClick = useCallback(item => setLastClicked(item), []);
    const elnURL = AppURL.create('notebooks').toHref();
    const canceledOnClick = useCallback((event: MouseEvent<HTMLButtonElement>) => {
        event.preventDefault();
        event.stopPropagation();
        console.log('Canceled event');
    }, []);
    const splitButtonClicked = useCallback(() => {
        console.log('Split Button has been clicked');
    }, []);
    const titleNode = (
        <span>
            <span className="fa fa-table" />
            <span> Dynamic Dropup</span>
        </span>
    );
    return (
        <div className="app-page">
            <div className="panel panel-default">
                <div className="panel-heading">Dropdown Menu</div>
                <div className="panel-body">
                    <DropdownButton title="Hard Coded Dropdown">
                        <MenuHeader text="Heading One" />
                        <MenuItem onClick={firstOnClick}>First Item</MenuItem>
                        <MenuItem href={elnURL}>Second Item</MenuItem>
                        <MenuItem>Third Item</MenuItem>
                        <MenuDivider />
                        <MenuHeader text="Heading Two" />
                        <MenuItem>Fourth Item</MenuItem>
                        <MenuItem>Fifth Item</MenuItem>
                        <MenuItem>Sixth Item</MenuItem>
                        <MenuDivider />
                        <MenuHeader text="Heading Three" />
                        <MenuItem>Seventh Item</MenuItem>
                        <MenuItem>Eighth Item</MenuItem>
                        <MenuItem>Ninth Item</MenuItem>
                    </DropdownButton>

                    <DropdownButton title="Hard Coded Dropdown noCaret pull right" noCaret pullRight>
            ...
            <div className="panel panel-default">
                <div className="panel-heading">Button That Cancels Events</div>
                <div className="panel-body">
                    <p>
                        This is a button that has a click handler that gets canceled when you click it:
                        <button type="button" className="btn btn-danger" onClick={canceledOnClick}>
                            Click me!
                        </button>
                    </p>
                    <p>Even though the event gets canceled we should still hopefully close any open menu on the page</p>
                </div>
            </div>
            ...
            <div className="panel panel-default">
                <div className="panel-heading">Dropdown Menu</div>
                <div className="panel-body">
                    <p>Last menu item clicked: {lastClicked}</p>
                    <DropdownButton title="Dynamic Dropdown">
                        {ITEMS.map((item, idx) => (
                            <Fragment key={item}>
                                {idx > 0 && idx % 3 === 0 && <MenuDivider />}
                                {idx % 3 === 0 && <MenuHeader text={'Header ' + (Math.floor(idx / 3) + 1)} />}
                                <DynamicMenuItem item={item} lastClicked={lastClicked} onClick={dynamicOnClick} />
                            </Fragment>
                        ))}
                    </DropdownButton>

                    <DropdownButton title="Disabled Dynamic Dropdown" disabled>
                        {ITEMS.map((item, idx) => (
                            <Fragment key={item}>
                                {idx > 0 && idx % 3 === 0 && <MenuDivider />}
                                {idx % 3 === 0 && <MenuHeader text={'Header ' + (Math.floor(idx / 3) + 1)} />}
                    ...
                    <DropdownButton title={titleNode} dropup>
                        {ITEMS.map((item, idx) => (
                            <Fragment key={item}>
                                {idx > 0 && idx % 3 === 0 && <MenuDivider />}
                                {idx % 3 === 0 && <MenuHeader text={'Header ' + (Math.floor(idx / 3) + 1)} />}
            ...
            <div className="panel panel-default">
                <div className="panel-heading">SplitButton Menu</div>
                <div className="panel-body">
                    <div>
                        <SplitButton onClick={splitButtonClicked} title="SplitButton Nothing Disabled">
                    ...
                    <div>
                        <SplitButton bsStyle="primary" onClick={splitButtonClicked} title="SplitButton Button Disabled" buttonDisabled>
                            <MenuItem>One</MenuItem>
                            <MenuItem>Two</MenuItem>
                            <MenuItem>Three</MenuItem>
                        </SplitButton>
                    ...
                    <div>
                        <SplitButton
                            bsStyle="danger"
                            onClick={splitButtonClicked}
                            title="SplitButton Menu Disabled"
                            menuDisabled
                        >
                            <MenuItem>One</MenuItem>
                            <MenuItem>Two</MenuItem>
                    ...
                    <div>
                        <SplitButton
                            bsStyle="success"
                            onClick={splitButtonClicked}
                            title="SplitButton Button and Menu Disabled"
                            buttonDisabled
                            menuDisabled
                        >
                            <MenuItem>One</MenuItem>
...
export const SchemaBrowserRoutes = () => (
    <Routes>
        <Route path="menus" element={<MenuExamples />} />
