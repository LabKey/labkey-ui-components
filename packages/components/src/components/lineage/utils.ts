...
export function applyMultiLineLabel(name: string, cpasType: string, isBold?: boolean): string {
    const cpasTypeEnd = cpasType ? cpasType.substring(cpasType.lastIndexOf(':') + 1) : '';
    let label = name;
...
export function processDepth(
    ...
    const depths = List<OrderedSet<string>>().asMutable();
    const processed = Set<string>().asMutable();
 ...
 */
function _processDepth(
    options: ILineageGroupingOptions,
    nodes: Map<string, LineageNode>,
    lsid: string,
    depths: List<OrderedSet<string>>,
    ...
    const queue = [];
    const edges = nodes.get(lsid).get(dir);
    if (edges.size > 0) {
        let depthSet;
