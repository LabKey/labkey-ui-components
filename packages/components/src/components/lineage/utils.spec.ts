...
describe('lineage utils', () => {
    ...
    test('getImageFromLineageNode', () => {
        ...
        lineageNode = lineageNode.set(
            'meta',
            LineageNodeMetadata.create(Map<any, any>(), QueryInfo.create({ iconURL: 'other' }))
        );
    ...
    test('getBackupImageFromLineageNode', () => {
        ...
        expect(getBackupImageFromLineageNode(lineageNode, false, false)).toBe(
            'https://labkey.org/_images/default_gray.svg'
        );

        lineageNode = LineageNode.create('test', { cpasType: 'SampleSet' });
        expect(getBackupImageFromLineageNode(lineageNode, false, false)).toBe(
            'https://labkey.org/_images/samples_gray.svg'
        );
    });
});
