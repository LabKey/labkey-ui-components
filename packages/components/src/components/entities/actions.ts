...
function getChosenParentData(model: EntityIdCreationModel, parentSchemaQueries: Map<string, EntityDataType>, allowParents: boolean) : Promise<Partial<EntityIdCreationModel>> {
    return new Promise((resolve, reject) => {
        let entityParents = EntityIdCreationModel.getEmptyEntityParents(
            parentSchemaQueries.reduce((names, entityDataType) => names.push(entityDataType.typeListingSchemaQuery.queryName), List<string>())
        );
                ...
                (chosenParents) => {
                    ...
                    if (validEntityCount === 1) {
                        resolve({
                            ...
                            entityParents: entityParents.set(parentSchemaQueries.get(parentRep.schema).typeListingSchemaQuery.queryName, chosenParents),
                        });
                    }
 ...
 */
export function getEntityTypeData(model: EntityIdCreationModel, schemaQueries: Map<string, EntityDataType>, targetQueryName: string, allowParents: boolean) : Promise<Partial<EntityIdCreationModel>> {
    return new Promise((resolve, reject) => {
        let promises = [];
        ...
        schemaQueries.forEach((entityDataType: EntityDataType, typeSchemaName: string) => {
