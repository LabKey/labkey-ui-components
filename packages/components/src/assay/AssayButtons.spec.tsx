...
import {
    ...
    AssayRunDetailHeaderButtons, UpdateQCStatesButton
} from './AssayButtons';
...
import {
    ...
    TEST_USER_PROJECT_ADMIN, TEST_USER_QC_ANALYST,
    TEST_USER_READER
...
describe("AssayDeleteBatchButton", () => {
   test("with batchId, can delete", () => {
       ...
       expect(menuItem.prop("href")).toBe("/labkey/experiment/deleteSelectedExperiments.view?singleObjectRowId=1&cancelUrl=%2F%23%2Fassays%2FGeneral%2FTestProtocol%2Fbatches%2F1&successUrl=%2F%23%2Fassays%2FGeneral%2FTestProtocol%2Fbatches");
   });
...
describe("AssayImportDataButton", () => {
    ...
    test("non-standard assay", () => {
        ...
        const button = wrapper.find(Button);
        expect(button).toHaveLength(1);
        expect(button.prop("href")).toBe("/labkey/Sample%20Management%202/assay-other.view?rowId=2&returnUrl=/#/assays/NAb/NAb%20Test/runs");
...
describe("UpdateQCStatesButton", () => {
    test("not analyst", () => {
        const wrapper = mountWithServerContext(
            <UpdateQCStatesButton disabled={false} onClick={jest.fn()} />,
            {
                user: TEST_USER_READER,
            }
        );
