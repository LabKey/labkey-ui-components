...
export class TimelineView extends React.Component<Props, any> {
    ...
    renderRow(event: TimelineEventModel, selectedEntityInfo?: any): any {
        ...
        if (selectedEntityInfo) {
            ...
            isConnection = selectedEntityInfo.firstEvent && selectedEntityInfo.lastEvent
                && (event.eventTimestamp >= selectedEntityInfo.firstEvent.eventTimestamp && event.eventTimestamp <= selectedEntityInfo.lastEvent.eventTimestamp);
        ...
        const icon = event.eventType === App.ASSAYS_KEY ? 'assay' : (event.eventType === 'inventory') ? 'default' : event.eventType;
        return (
            <tr key={event.getRowKey()}
                onClick={() => {
    ...
    renderTimestampCol(timestamp) {
        return (<td key={"tl-timestamp-col"} className={'display-light timeline-timestamp-col'}>{getEventDataValueDisplay(timestamp)}</td>);
    }

    renderIconCol(iconSrc: string, isSelected?: boolean, isFirstEvent?: boolean, isLastEvent?: boolean, isClosedEvent?: boolean, isConnection?: boolean) {
        ...
        return (<td key={"tl-icon-col"} className={"icon-col"}>
            <div><div className="timeline-line">{line}</div>{icon}</div>
        </td>);
    }

    renderDetailCol(summary: string, user: any, entity: any) {
        const { showUserLinks } = this.props;
        return (<td key={"tl-detail-col"} className={'detail-col'}>
            <div>
