...
describe('getSampleFinderCommonConfigs', () => {
    test('No cards', () => {
        expect(getSampleFinderCommonConfigs([])).toStrictEqual({baseFilters: [], requiredColumns: SAMPLE_STATUS_REQUIRED_COLUMNS});
    });

    test('Cards without filters', () => {
        expect(getSampleFinderCommonConfigs([{
    ...
    test('Cards with and without filters', () => {
        ...
        expect(getSampleFinderCommonConfigs([
            ...
            baseFilters: [Filter.create('QueryableInputs/Materials/TestQuery/Name', null, Filter.Types.NONBLANK), cardFilter],
            requiredColumns: [...SAMPLE_STATUS_REQUIRED_COLUMNS, 'QueryableInputs/Materials/TestQuery','QueryableInputs/Materials/TestQuery2/TestColumn' ]
        });
    });
});
...
describe("getSampleFinderQueryConfigs", () => {
    ...
    test('no sample type names, no cards', () => {
        expect(getSampleFinderQueryConfigs(TEST_USER_EDITOR, [], [], 'testId'))
            .toStrictEqual({
    ...
    test("no sample type names", () => {
        expect(getSampleFinderQueryConfigs(TEST_USER_EDITOR, [], [
            {
                entityDataType: TestTypeDataType,
                schemaQuery: SchemaQuery.create("Samples", "TestQuery")
    ...
    test("multiple sample types, no cards", () => {
        expect(getSampleFinderQueryConfigs(TEST_USER_GUEST, ["Sample Type 1", "Sample Type 2"], [], 'testId'))
            .toStrictEqual({
                'testId|exp/materials': {
                    id: 'testId|exp/materials',
                    title: 'All Samples',
    ...
    test("with names and cards", () => {
        ...
        expect(getSampleFinderQueryConfigs(TEST_USER_GUEST, ["Sample Type 1", "Sample Type 2"], cards, 'testId'))
            .toStrictEqual({
                'testId|exp/materials': {
                    id: 'testId|exp/materials',
                    title: 'All Samples',
