...
export class BarTenderSettingsFormImpl extends React.PureComponent<Props, State> {
    ...
    onSave = () => {
        ...
        saveBarTenderConfiguration(config).then((btConfig: BarTenderConfiguration): void => {
    ...
    onVerifyBarTenderConfiguration = () => {
        ...
        printBarTenderLabels(this.btTestConnectionTemplate(defaultLabel), btServiceURL).then((btResponse:BarTenderResponse) => {
            ...
            <Row className="form-group">
                <Col xs={12}>
                    <div className={'pull-right'}>
                        <HelpLink topic={BAR_TENDER_TOPIC} className="label-printing--help-link">Learn more about BarTender</HelpLink>
                    </div>
                    <div>
                        {label} {description && <LabelHelpTip title={label}><p>{description}</p></LabelHelpTip>}
                    </div>
                </Col>
                <Col xs={12}>
                    <FormControl
    ...
    renderLabelInput = () => {
        ...
        const description = 'Default Label file to use when printing labels with BarTender. The path should be relative to the default folder configured for the BarTender web service.';
    ...
    private renderConnectionFailedValidation(): React.ReactNode {
        ...
        return (
            <Alert bsStyle="danger">{failureMessage}</Alert>
        );
    }
