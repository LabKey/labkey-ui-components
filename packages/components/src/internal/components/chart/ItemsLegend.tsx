    ...
    render() {
        ...
        legendData.forEach((legend, index) => {
            ...
            if (legend.circleColor && legend.circleColor !== 'none') {
                icon = <i className="color-icon__circle cell-legend-circle" style={{ backgroundColor: legend.circleColor }} />;
            }
            ...
            const legendDisplay = (
                <tr key={key} className={'cell-legend-row'}>
                    <td>
                        <span className={classNames("cell-legend-icon", {
                            "cell-legend-icon-border": hasBackground
                        })} style={{ backgroundColor: hasBackground ? legend.backgroundColor : emptyColor}}>{icon}
                        </span>
                    </td>
                    <td><span className={'cell-legend-label'}>{legend.legendLabel}</span></td>
                </tr>
            );
