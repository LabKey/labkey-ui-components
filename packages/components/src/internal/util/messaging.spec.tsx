...
describe('resolveErrorMessage', () => {
    ...
    test('duplicate key violation exception - Postgres', () => {
        ...
        expect(resolveErrorMessage(error, 'samples', undefined)).toBe(
            "There was a problem creating your samples. Duplicate name 'E-20200121-743' found."
        );
    });
    ...
    test('duplicate key violation exception - SQLServer', () => {
        const error = {
            exception:
                "Violation of UNIQUE KEY constraint 'UQ_Material_LSID'. Cannot insert duplicate key in object 'exp.Material'. The duplicate key value is (urn:lsid:labkey.com:Sample.9928.Second%2520Sample%2520Set:S-1, S-1).",
        };
        expect(resolveErrorMessage(error, 'samples', undefined)).toBe(
            "There was a problem creating your samples. Duplicate name 'S-1' found."
        );
    });
