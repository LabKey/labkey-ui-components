...
export class AuditDetails extends React.Component<Props, State> {
    ...
    renderDeleteValue(oldVal: string, newVal: string) {
        return (
            <span className={'display-light old-audit-value'}>{oldVal}</span>
        );
    }
    ...
    getValueDisplay = (field : string, value : string) : any => {
        ...
        if (fieldValueRenderer)
            displayVal = fieldValueRenderer(field, value, displayVal);
    ...
    renderRow(field: string, oldVal: string, newVal: string, isUpdate: boolean, isInsert: boolean) {
        ...
        let valueRenderer;
        if (isUpdate)
            valueRenderer = this.renderUpdateValue;
        else if (isInsert)
            valueRenderer = this.renderInsertValue;
        else
            valueRenderer = this.renderDeleteValue;

        if (!user.isSignedIn && AuditDetails.isUserFieldLabel(field))
            return null;

        return (
            <Row className={'margin-bottom'} key={field}>
                <Col className={'left-spacing right-spacing'}>
                    <span className={'audit-detail-row-label job-field-label right-spacing'}>{capitalizeFirstChar(field)}:</span>
                </Col>
                <Col className={'left-spacing right-spacing'}>
    ...
    getUserDisplay(userId: number, showUserLink: boolean) {
        ...
        else // user may have been deleted
            return (<span className={'empty-section'} title={'User deleted from server'}>{'<' + userId + '>'}</span>)
    }
