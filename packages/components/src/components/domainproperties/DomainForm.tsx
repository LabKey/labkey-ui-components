...
export class DomainFormImpl extends React.PureComponent<IDomainFormInput, IDomainFormState> {
        ...
        if (this.shouldShowInferFromFile()) {
            ...
            return (
                <Row className="domain-add-field-row">
                    <Col xs={12}>
                        <AddEntityButton entity="Field" buttonClass="domain-form-add-btn" onClick={this.onAddField} />
                    </Col>
                </Row>
            );
        }
    ...
    getPanelHeaderClass(): string {
        ...
        classes +=
            this.isPanelExpanded() && controlledCollapse
                ? ' domain-panel-header-expanded'
                : ' domain-panel-header-collapsed';
    ...
    getAlertClasses = () => {
        ...
        if (!collapsed) classes += ' domain-bottom-alert-top';
    ...
    renderHeaderContent() {
        ...
        return (
            <>
                ...
                {iconHelpMsg && (
                    <LabelHelpTip
                        title={DomainFormImpl.getHeaderName(domain.name, headerTitle, headerPrefix)}
                        body={() => iconHelpMsg}
                        placement="top"
                ...
                {/* Header name*/}
                <span className="domain-panel-title">
                    {DomainFormImpl.getHeaderName(domain.name, headerTitle, headerPrefix)}
                </span>
                ...
                {controlledCollapse && domain.fields.size > 0 && (
                    <span className="domain-panel-header-fields-defined">
                ...
                <Panel className={this.getPanelClass()} expanded={this.isPanelExpanded()} onToggle={function() {}}>
                    {showHeader && (
                        <Panel.Heading
                            ...
                            id={
                                domain && domain.name
                                    ? createFormInputName(domain.name.replace(/\s/g, '-') + '-hdr')
                                    : 'domain-header'
                            }>
