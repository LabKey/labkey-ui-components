...
describe('DomainField', () => {
    ...
    test('getDetailsTextArray', () => {
        ...
        expect(field.getDetailsTextArray(0).slice(0, 2).join('')).toBe('Updated. Current Folder > exp');
        expect(field.getDetailsTextArray(0)[3].props.href).toBe('/labkey/query/testContainer/executeQuery.view?schemaName=exp&query.queryName=SampleType1');

        field = field.merge({ lookupContainer: 'Test Folder' }) as DomainField;
        expect(field.getDetailsTextArray(0).slice(0, 2).join('')).toBe('Updated. Test Folder > exp');
        expect(field.getDetailsTextArray(0)[3].props.href).toBe('/labkey/query/Test%20Folder/executeQuery.view?schemaName=exp&query.queryName=SampleType1');
