...
const PicklistGridImpl: FC<PicklistGridProps & InjectedQueryModels> = memo(props => {
    ...
    const onPicklistDelete = (listsToDelete: Picklist[]) => {
        ...
        deletePicklists(listsToDelete)
            ...
            .catch(reason => {
                setShowDeleteModal(false);
                createNotification({
                    message: "There was a problem deleting the selected pick" + noun + ".  Be sure the " + (listsToDelete.length === 1 ? noun + " is still valid and has" : noun + " are still valid and have") + " not been deleted by another user.",
                    alertClass: "danger"
                })
            });
    }
    ...
    return (
        <Section
            ...
            context={CreateButton !== undefined && (
...
export const PicklistListing: FC<OwnProps> = memo(props => {
    ...
    const queryConfigs = {
        [MY_PICKLISTS_GRID_ID]: {
            baseFilters: [
                Filter.create('CreatedBy', user.id)
            ],
        ...
        [TEAM_PICKLISTS_GRID_ID]: {
            baseFilters: [
                Filter.create("Category", PUBLIC_PICKLIST_CATEGORY),
            ],
            sorts: [new QuerySort({ fieldKey: 'Name' })],
