...
describe("<SampleEventListing/>", () => {
    ...
    test('Without selected event', () => {
        const tree = renderer.create(<SampleEventListing
            sampleId={86873}
    ...
    test('Sort by latest', () => {
        const comp = shallow(<SampleEventListing
            sampleId={86873}
            sampleName={'S-20200404-1'}
            showUserLinks={true}
            onEventSelection={jest.fn()}
    ...
    test('Filter by event type and date', () => {
        ...
        comp.instance().setState({filterExpanded: true, includeJobEvent: true, filterStartDate: new Date('2020-04-09 00:00:00.000 UTC')});
        comp.update();
