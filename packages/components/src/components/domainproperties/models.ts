...
export class DomainException
    ...
    static create(rawModel: any, severityLevel): DomainException {
        if (rawModel && rawModel.exception) {
            ...
            const exception = this.getExceptionMessage(errors);
...
export class AssayProtocolModel extends Record({
    ...
    static isValid(model: AssayProtocolModel): boolean {
        const errDomain = model.domains.find(
            dom => !!dom.domainException && dom.domainException.severity === SEVERITY_LEVEL_ERROR
        );
