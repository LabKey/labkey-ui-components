...
export function renderSampleRequiredPanelHeader(config: IAppDomainHeader) {
    return <SampleRequiredDomainHeader {...config} />;
}
...
export class SampleRequiredDomainHeader extends React.PureComponent<IAppDomainHeader> {
    ...
    render() {
        ...
        if (!domain || !domain.fields || domain.fields.size == 0) return null;
        ...
        return (
            <>
                <Alert bsStyle="info">
                    <Row>
                        <Col xs={12}>
                            {REQUIRED_SAMPLE_FIELD_MSG} This allows exploration of consolidated data in a way that
                            provides greater insight into the connections between your samples and assay data. You can
                            either select one of your existing fields from the input below to set it as the sample
                            lookup, or you can add a new field of type "Sample".
                        </Col>
                    </Row>
                    <Row className="margin-top">
                        <Col xs={3}>Map field as Sample Lookup</Col>
                        <Col xs={6}>
                            <FormControl
                            ...
                            >
                                ...
                                <option value={ADD_NEW_FIELD_INDEX} key={ADD_NEW_FIELD_INDEX}>
                                    Add new field
                                </option>
                                {domain.fields.map((field: DomainField, idx: number) => {
                                    const { rangeURI, name } = field;
                                    if (!field.isNew() && rangeURI !== RANGE_URIS.INT) return;
