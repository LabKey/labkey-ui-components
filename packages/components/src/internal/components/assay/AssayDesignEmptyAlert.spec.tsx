...
describe('AssayDesignEmptyAlert', () => {
    ...
    test('with permissions', () => {
        const wrapper = mountWithAppServerContext(<AssayDesignEmptyAlert user={TEST_USER_ASSAY_DESIGNER} />, {}, homeProjectContext);
    ...
    test('without permissions', () => {
        ...
        const wrapper = mountWithAppServerContext(<AssayDesignEmptyAlert message={expectedMessage} user={TEST_USER_READER} />, {}, homeProjectContext);

        expect(wrapper.find(EMPTY_ALERT).at(0).text()).toEqual(expectedMessage);
    });
    test('top level folder context', () => {
        const wrapper = mountWithAppServerContext(<AssayDesignEmptyAlert user={TEST_USER_ASSAY_DESIGNER} />, {}, topFolderContext);

        // Expect link to design
        expect(wrapper.find(`${EMPTY_ALERT} a`).prop('href')).toEqual(NEW_ASSAY_DESIGN_HREF.toHref());
    });
    test('child project folder context', () => {
        const wrapper = mountWithAppServerContext(<AssayDesignEmptyAlert user={TEST_USER_ASSAY_DESIGNER} />, {}, childProjectContext);
    ...
    test('child folder but Projects feature not enabled for folder', () => {
        const wrapper = mountWithAppServerContext(<AssayDesignEmptyAlert user={TEST_USER_ASSAY_DESIGNER} />, {}, childFolderNonProjectContext);
