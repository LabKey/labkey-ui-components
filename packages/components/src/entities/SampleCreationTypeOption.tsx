...
import { SampleCreationTypeModel } from '../internal/components/samples/models';
        ...
        <div onClick={onClick} className={classNames('creation-type', { selected: isSelected })}>
            {showIcon && (
                <div className="creation-type-icon">
                    ...
                    {option.iconSrc && (
                        <SVGIcon iconSrc={option.iconSrc} theme={isSelected ? Theme.DEFAULT : Theme.GRAY} />
                    )}
                </div>
            )}
            <div className={classNames('creation-type-choice', { selected: isSelected })}>
                <input checked={isSelected} type="radio" name="creationType" value={option.type} onChange={onClick} />{' '}
                {option.type}
                <div className="creation-type-choice-description">{option.description}</div>
            </div>
        </div>
    );
});
